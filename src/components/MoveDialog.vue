<template>
  <OsModal
    ...
    size="lg"
    closable
    @update:visible="v => v === false && onCancel()"
  >
    <!-- Source context -->
    ...
    <div class="move-summary">
      ...
      <div v-if="conflicts.length > 0" class="move-conflicts">
        <div v-for="c in conflicts" :key="c.id" class="conflict-row">
          WARNING: {{ c.name }} already exists at destination
        </div>
      </div>
    </div>

    <template #footer>
      <OsButton variant="ghost" size="sm" :disabled="isMoving" @click="onCancel">CANCEL</OsButton>
      <OsButton
        v-if="!selectedDest"
        variant="ghost"
        size="sm"
        disabled
      >SELECT A DESTINATION</OsButton>
      <OsButton
        v-else
        variant="neon"
        size="sm"
...
<style scoped>
.move-source { padding: 0 0 8px; }
.move-section-title { font-size: var(--font-size-xs); font-weight: 700; color: var(--text-muted); letter-spacing: 0.5px; margin-bottom: 6px; }
.move-source-info { display: flex; align-items: center; gap: 4px; padding: 3px 8px; font-size: var(--font-size-sm); color: var(--text-muted); margin-bottom: 4px; }
.move-file-list { display: flex; flex-direction: column; gap: 2px; max-height: 110px; overflow-y: auto; }
.move-file-row { display: flex; align-items: center; gap: 6px; padding: 4px 8px; background: var(--bg-surface); border-radius: var(--radius-sm); font-size: var(--font-size-sm); color: var(--text-secondary); transition: background 0.15s; }
.move-file-row.file-error { background: var(--danger-dim); }
.move-file-row.file-done { background: var(--accent-dim); }
.row-icon { flex-shrink: 0; color: var(--text-muted); }
.move-file-name { flex: 1; min-width: 0; }
.row-size { font-size: var(--font-size-xs); color: var(--text-muted); flex-shrink: 0; }
.row-sync-badges { display: flex; gap: 2px; margin-left: 4px; }
.sync-badge { display: flex; align-items: center; padding: 1px 4px; background: var(--accent-dim); border: 1px solid rgba(0,255,65,0.1); border-radius: 3px; color: var(--text-accent); font-size: 7px; }
.row-status { font-size: 7px; color: var(--text-muted); margin-left: 4px; font-weight: 700; }
.move-op-type { padding: 8px 0; }
.op-type-options { display: flex; gap: 4px; margin-bottom: 6px; }
.op-btn { flex: 1; display: flex; align-items: center; justify-content: center; gap: 4px; padding: 6px 8px; background: var(--bg-surface); border: 1px solid var(--border-medium); border-radius: var(--radius-sm); color: var(--text-muted); font-family: var(--font-mono); font-size: var(--font-size-sm); font-weight: 700; cursor: pointer; transition: all var(--transition-fast); }
.op-btn:hover { border-color: var(--border-strong); color: var(--text-secondary); }
.op-btn.active { border-color: var(--accent); color: var(--text-accent); background: var(--accent-dim); }
.op-btn-transfer.active { border-color: #45B7D1; color: #45B7D1; background: rgba(69,183,209,0.04); }
.op-desc { font-size: var(--font-size-xs); color: var(--text-muted); padding: 0 2px; }
.move-dest-section { padding: 8px 0; }
.move-dest-group { margin-bottom: 6px; }
.move-group-label { font-size: var(--font-size-xs); color: var(--text-muted); margin-bottom: 3px; letter-spacing: 0.3px; }
.move-folder-list { display: flex; flex-direction: column; gap: 2px; max-height: 130px; overflow-y: auto; }
.move-option { display: flex; align-items: center; gap: 6px; padding: 4px 6px; cursor: pointer; border-radius: var(--radius-sm); font-size: var(--font-size-sm); color: var(--text-secondary); border: 1px solid transparent; transition: all 0.08s; }
.move-option:hover { background: var(--bg-overlay); border-color: var(--border-medium); }
.move-option.active { border-color: rgba(0,255,65,0.3); background: var(--accent-dim); }
.move-radio { appearance: none; width: 12px; height: 12px; border: 1px solid var(--text-muted); background: var(--bg-surface); cursor: pointer; border-radius: 50%; flex-shrink: 0; }
.move-radio:checked { background: var(--accent); border-color: var(--accent); }
.dest-icon { flex-shrink: 0; color: var(--text-muted); }
.move-folder-name { flex: 1; min-width: 0; }
.dest-disabled { font-size: var(--font-size-xs); color: var(--text-danger); }
.dest-extra { font-size: 7px; color: var(--text-muted); }
.no-items-hint { font-size: var(--font-size-sm); color: var(--text-muted); padding: 4px 6px; }
.move-summary { min-height: 28px; display: flex; flex-direction: column; gap: 4px; }
.move-summary-line { display: flex; align-items: center; gap: 6px; font-size: var(--font-size-sm); color: var(--text-muted); }
