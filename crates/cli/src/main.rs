...
use cybermanju_crypto::{
    decrypt_data, encrypt_data, EncryptedFileMeta, EncryptionAlgo, KeyPair, PqcEngine,
};
    ...
    while let Ok(msg) = rx.recv() {
        match msg {
            ...
            tui::TaskMessage::HarvestComplete(_, files, bytes) => {
                println!(
                    " \x1b[32mDone — {} files, {} bytes harvested\x1b[0m",
                    files, bytes
                );
                break;
            }
...
fn cmd_test_connection(name: &str) -> Result<()> {
    ...
    match be.test_connection() {
        ...
        Ok(false) => {
            err_msg("Connection failed (unexpected response)");
            Ok(())
        }
        Err(e) => {
            err_msg(&format!("Connection error: {}", e));
            Ok(())
        }
    }
}
...
fn cmd_transfer_cli(source: &str, dest: &str) -> Result<()> {
    ...
    println!(
        " {} -> {} transfer initiated (use `cybermanju tui` for interactive)",
        source, dest
    );
    let (tx, rx) = std::sync::mpsc::channel();
    let backends = cfg.backends.clone();
        ...
        PortableAction::Create { path } => {
            ...
            while let Ok(msg) = rx.recv() {
                match msg {
                    ...
                    tui::TaskMessage::PortableDone(s) => {
                        println!(" \x1b[32m{}\x1b[0m", s);
                        break;
                    }
                    tui::TaskMessage::PortableError(e) => {
...
fn check_data_dir() {
    ...
    if db_path.exists() {
        ...
        if let Some(m) = meta {
            kv(
                "Database",
                &format!("{} ({} MB)", db_path.display(), m.len() / 1024 / 1024),
            );
        }
    } else {
...
fn check_env_deps() {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if wv2.exists() {
            ...
            .join("Microsoft\\Edge\\Application\\webview2.exe");
            if alt.exists() {
                ok("WebView2 found (alt)");
            } else {
                warn("WebView2 not detected");
            }
        }
    }
    #[cfg(target_os = "macos")]
...
fn cmd_encrypt(path: PathBuf, output: Option<PathBuf>) -> Result<()> {
    ...
    let key = if kd.exists() {
        ...
        if let Some(entry) = entries.last() {
            ...
            warn("No saved keys — generating ephemeral");
            let mut engine = PqcEngine::new();
            engine.generate_keypair(EncryptionAlgo::Kyber1024)?
        }
    } else {
        warn("No keys dir — generating ephemeral key");
        let mut engine = PqcEngine::new();
        engine.generate_keypair(EncryptionAlgo::Kyber1024)?
    };
    let encrypted =
        encrypt_data(&data, &key).map_err(|e| anyhow::anyhow!("Encryption failed: {}", e))?;
...
fn cmd_decrypt(path: PathBuf, output: Option<PathBuf>, key_id: &str) -> Result<()> {
    ...
    let meta_line = lines
        .next()
        .ok_or_else(|| anyhow::anyhow!("Invalid encrypted file"))?;
    ...
    let plaintext =
        decrypt_data(&encrypted, &key).map_err(|e| anyhow::anyhow!("Decryption failed: {}", e))?;
    let out_path = output.unwrap_or_else(|| {
        let mut p = path.clone();
...
fn cmd_keys(action: KeyAction) -> Result<()> {
    match action {
        KeyAction::List => {
            ...
            let kd = keys_dir();
            if !kd.exists() {
                info("No keys found. Generate one with: cybermanju keys generate");
                return Ok(());
            }
                ...
                if path.extension().map_or(false, |ext| ext == "json") {
                    if let Ok(data) = std::fs::read_to_string(&path) {
                        if let Ok(kp) = serde_json::from_str::<KeyPair>(&data) {
                            kv(
                                &kp.id,
                                &format!("{:?} — created {}", kp.algorithm, kp.created_at),
                            );
                        }
                    }
                }
        ...
        KeyAction::Show { id } => {
            ...
            kv(
                "Public key",
                &base64::engine::general_purpose::STANDARD.encode(&key.public_key)[..48],
            );
    ...
    match action {
        DbAction::Check => {
            ...
            if !db_path.exists() {
                ...
                info("Run the desktop app first");
                return Ok(());
            }
...
mod tests {
    ...
    fn test_clap_parses_portable_create() {
        let cli = Cli::try_parse_from(["cybermanju", "portable", "create", "/tmp/test.cybermanju"])
            .unwrap();
    ...
    fn test_clap_parses_transfer() {
        let cli =
            Cli::try_parse_from(["cybermanju", "transfer", "source-name", "dest-name"]).unwrap();
