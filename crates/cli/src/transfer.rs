...
pub fn run_transfer(
    ...
    let src = match backends.get(src_idx) {
        Some(s) => s,
        None => {
            let _ = tx.send(TaskMessage::TransferError("invalid source".into()));
            return;
        }
    };
    let dst = match backends.get(dst_idx) {
        Some(d) => d,
        None => {
            let _ = tx.send(TaskMessage::TransferError("invalid destination".into()));
            return;
        }
    };
    ...
    let src_be = match create_backend(src.backend_type, &src.token, &src.config) {
        Some(b) => b,
        None => {
            let _ = tx.send(TaskMessage::TransferError(
                "cannot create source backend".into(),
            ));
            return;
        }
    };
    let dst_be = match create_backend(dst.backend_type, &dst.token, &dst.config) {
        ...
        None => {
            let _ = tx.send(TaskMessage::TransferError(
                "cannot create destination backend".into(),
            ));
            return;
        }
    };
    ...
    let files = match src_be.list_files("") {
        Ok(f) => f,
        Err(e) => {
            let _ = tx.send(TaskMessage::TransferError(format!("list source: {}", e)));
            return;
        }
    };
    ...
    let _ = tx.send(TaskMessage::TransferProgress(
        1.0,
        format!(
            "transferred {} files from {} to {}",
            total, src.name, dst.name
        ),
    ));
