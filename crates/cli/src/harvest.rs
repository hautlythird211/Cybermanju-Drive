...
pub fn run_harvest_with_output(
    backends: Vec<StoredBackend>,
    ...
    for sb in &backends {
        let be = match create_backend(sb.backend_type, &sb.token, &sb.config) {
            Some(b) => b,
            None => {
                let _ = tx.send(TaskMessage::HarvestError(
                    seq,
                    sb.name.clone(),
                    "failed to create backend".into(),
                ));
                completed += 1;
                continue;
            }
            ...
            match be.download_file(&f.path, local.to_str().unwrap_or("")) {
                Ok(_) => {
                    ...
                    let _ = tx.send(TaskMessage::HarvestProgress(
                        ...
                        total_files,
                        dl_bytes,
                        format!("{}/{} files", dl_ok, total_files),
                    ));
                }
                Err(e) => {
                    grand_errors += 1;
                    let _ = tx.send(TaskMessage::HarvestError(
                        seq,
                        sb.name.clone(),
                        format!("download error for {}: {}", f.name, e),
                    ));
                }
            }
        }

        let _ = tx.send(TaskMessage::HarvestDone(
            seq,
            sb.name.clone(),
            dl_ok,
            dl_bytes,
        ...
        let _ = tx.send(TaskMessage::HarvestOverall(
            seq,
            pct,
            format!("{}/{} backends done", completed, total),
        ));
    }
    ...
    match cybermanju_portable_db::PortableDatabase::create(
                                ...
                                if path.is_file() {
                                    if let Ok(data) = fs::read(&path) {
                                        ...
                                        let name = path
                                            ...
                                            .to_string_lossy()
                                            .to_string();
                                        let _ = pdb.store_compressed_content(
                                            &db, &file_id, &data, &name, None,
                                        );
                                    }
                                }
                            }
    ...
    let _ = tx.send(TaskMessage::HarvestOverall(
        seq,
        1.0,
        format!("done — output: {}", pdb_path.display()),
    ));
