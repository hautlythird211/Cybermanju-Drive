...
pub fn create_archive(path: &str, tx: Sender<TaskMessage>) {
    let _ = tx.send(TaskMessage::PortableProgress(0.0, "creating .cybermanju archive...".into()));
    match PortableDatabase::create(path, "cli") {
        Ok(_) => {
            let _ = tx.send(TaskMessage::PortableProgress(1.0, format!("created: {}", path)));
...
pub fn extract_archive(path: &str, tx: Sender<TaskMessage>) {
    let _ = tx.send(TaskMessage::PortableProgress(0.0, "opening .cybermanju...".into()));
    ...
    let _ = tx.send(TaskMessage::PortableProgress(0.3, "unpacking database...".into()));

    let out_path = format!("{}.unpacked.db", path);
    match pdb.unpack(&out_path, None) {
        Ok(db) => {
            let header = pdb.header();
            let _ = tx.send(TaskMessage::PortableProgress(0.6, format!("db: {} files, {} relations, {} deletions", header.total_files, header.total_relations, header.total_deletions)));
            ...
            let _ = tx.send(TaskMessage::PortableProgress(0.7, format!("extracting {} files...", total)));

            for (i, entry) in entries.iter().enumerate() {
                if let Ok(Some((data, mime))) = pdb.get_recoverable_data(&db, &entry.original_file_id) {
                    let fpath = Path::new(&extract_dir).join(&entry.original_name);
                    let _ = fs::write(&fpath, &data);
                }
                let pct = 0.7 + (i as f64 / total as f64) * 0.25;
                let _ = tx.send(TaskMessage::PortableProgress(pct, format!("extracting {}/{}", i + 1, total)));
            }

            let _ = tx.send(TaskMessage::PortableProgress(1.0, format!("extracted {} files to {}", total, extract_dir)));
            let _ = tx.send(TaskMessage::PortableDone(format!("Extracted {} files to {} (db: {})", total, extract_dir, out_path)));
        }
        Err(e) => {
