...
impl StorageBackend for TelegramBackend {
    ...
    fn upload_file(&self, local_path: &str, _remote_path: &str) -> Result<String, String> {
        ...
        let file_id = v["result"]["document"]["file_id"].as_str().unwrap_or("").to_string();
        if file_id.is_empty() {
            // Try photo/audio/video fallback
            let fallback = v["result"]["photo"].as_array().and_then(|arr| arr.last())
                .and_then(|p| p["file_id"].as_str())
                .or_else(|| v["result"]["audio"]["file_id"].as_str())
                .or_else(|| v["result"]["video"]["file_id"].as_str())
                .unwrap_or("");
            if fallback.is_empty() {
                return Err("No file_id in Telegram response (media type may not be supported)".to_string());
            }
    ...
    fn download_file(&self, remote_path: &str, local_path: &str) -> Result<(), String> {
        ...
        let dl = client
            .get(&format!("https://api.telegram.org/file/bot{}/{}", self.bot_token, file_path))
            .send()
                ...
                if !doc.is_null() {
                    files.push(RemoteFile {
                        ...
                        modified_at: msg["message"]["date"]
                            .as_i64()
                            .and_then(|d| chrono::DateTime::from_timestamp(d, 0).map(|t| t.to_rfc3339()))
                            .unwrap_or_default(),
    ...
    fn test_connection(&self) -> Result<bool, String> {
        ...
        if status != 200 {
            return Err(format!("Telegram connection test failed (HTTP {}): {}", status, body));
        }
        ...
        let bot_name = resp_json["result"]["username"].as_str().unwrap_or("unknown");
